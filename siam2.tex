...
a technique known as \emph{kriging }(CITE)\emph{. }Computing the optimal
...
to somewhat weak interactions between measurements (which is verified
in some contexts, see for example CITEBARRY). Determinants also play
a crucial role in quantum physics, and especially in quantum chromatography
(QC) and lattice theory. The interaction between particles can be
approximated as a lattice, and the energy level of particles is the
determinant of some functional operators over this lattice (Slatter
Determinant, CITE). It is itself a very complex problem because of
the size of the matrices involved for any non-trivial problem. In
this respect, the restriction to diagonally dominant matrices can
