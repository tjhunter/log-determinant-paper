...
with $\kappa_{C}$ the constant introduced in Lemma 4.5 in \cite{Koutis2011}.
This coarse analysis gives us the bound:
\[
\kappa\left(B_{i}\right)\leq\tilde{O}\left(\kappa_{r}^{\log n}\log^{2}n\kappa\left(A_{1}\right)\right)=\tilde{O}\left(n^{\log\kappa_{r}}\log^{2}n\kappa\left(A_{1}\right)\right)
\]
 for some absolute constant $\kappa_{r}$. Consider the relative precision
$\tilde{\nu}=\tilde{\mathcal{O}}\left(n^{-\log\kappa_{r}}\log^{-8}n\frac{\epsilon}{\kappa\left(A_{1}\right)}\right)$
