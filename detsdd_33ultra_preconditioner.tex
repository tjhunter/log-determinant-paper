...
\begin{definition} Definition 4.2 from \cite{Koutis2011}. Good preconditioning
...
% TODO: the newer version from Koutis2011 has a  good chain, but then it produces  I think. See the proof of their lemma 4.5
...
produces a $\tilde{O}\left(\log^{2}n\right)$-good chain for $A$
with probability $1-p$. The algorithm runs in expected time: 
\[
...
is upper bounded.
\end{itemize}
As a consequence, we have for all $i$: $\kappa\left(A_{i+1}\right)\leq\kappa\left(B_{i}\right)\leq\kappa_{i}\kappa\left(A_{i}\right)\leq\prod_{j=1}^{j}\kappa_{i}\kappa\left(A_{1}\right)\leq\tilde{O}\left(\kappa_{C}^{i-1}\log^{2}n\kappa\left(A_{1}\right)\right)$.
This coarse analysis gives us the bound:
\[
\kappa\left(B_{i}\right)\leq\tilde{O}\left(\kappa_{D}^{\log n}\log^{2}n\kappa\left(A_{1}\right)\right)=\tilde{O}\left(n^{\log\kappa_{D}}\log^{2}n\kappa\left(A_{1}\right)\right)
\]
 for some absolute constant $\kappa_{D}$. Consider the relative precision
$\tilde{\nu}=\tilde{\mathcal{O}}\left(n^{-\log\kappa_{D}}\log^{-8}n\frac{\epsilon}{\kappa\left(A_{1}\right)}\right)$
...
by assuming that $\epsilon\geq n^{-1}$, which then becomes $\tilde{O}\left(m\epsilon^{-1}\log^{2}n\log^{2}\left(\frac{\kappa\left(B\right)}{\epsilon}\right)\log\left(\eta^{-1}\right)\right)$. 
