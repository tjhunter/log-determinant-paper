...
\begin{lemma}\label{lem:nu-sequence-bound}Let $H$ be a linear operator
and $D$ a norm over the space of that linear operator. Assume that
the operator $H$ is a contraction under this norm ($\left\Vert H\right\Vert _{D}<1$)
