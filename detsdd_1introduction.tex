...
\[
\tilde{O}\left(m\epsilon^{-1}\log^{3}n\log^{2}\left(\frac{n\kappa\left(B\right)}{\epsilon}\right)\right)
\]
...
\textbf{Applications.} The problem of estimating determinants has
...
\]
and this algorithm completes in expected time $\tilde{O}\left(m\epsilon^{-1}\log^{2}n\log^{2}\left(\frac{\kappa\left(B\right)}{\epsilon}\right)\log\left(\eta^{-1}\right)\right)$.

\end{theorem}

\textbf{A note on scaling.} Unlike other common characteristics of
